...
class BandPackageReader(MultiBandReader):
    band_package: Sequence[typing.Tuple[str, typing.Union[str, bytes, os.PathLike]]] = attr.ib(init=False)
    ...
    def __attrs_post_init__(self):
        self.bands = sorted(pack[0] for pack in self.band_package)
        self.paths = sorted(pack[1] for pack in self.band_package)

        first_band_path = self._get_band_url(self.bands[1])
        with self.reader(first_band_path) as cog:
